...
import './ViewAppointments.css';
import { FaUserMd, FaCalendarAlt, FaClock, FaCheckCircle, FaTimesCircle, FaEdit, FaTrash } from 'react-icons/fa';
  ...
  useEffect(() => {
    const fetchAppointments = async () => {
      try {
        ...
        const active = data.filter(appointment => appointment.status !== 'canceled');
        const canceled = data.filter(appointment => appointment.status === 'canceled');
  ...
  const cancelAppointment = async (id) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setActiveAppointments(prev => prev.filter(appointment => appointment._id !== id));
...
{activeAppointments.length > 0 ? (
  <ul>
    {activeAppointments.map((appointment) => (
      <li key={appointment._id} className="ViewAppointments-appointment-item">
        <div className="ViewAppointments-doctor-info">
          ...
          <div className="ViewAppointments-appointment-details">
            <p><FaUserMd /> <strong>Doctor:</strong> {appointment.doctorId.fname} {appointment.doctorId.lname}</p>
            <p><FaCalendarAlt /> <strong>Date:</strong> {new Date(appointment.date).toLocaleDateString()}</p>
            <p><FaClock /> <strong>Time:</strong> {new Date(appointment.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
            <p><FaCheckCircle /> <strong>Status:</strong> Active</p>
          </div>
        </div>
      ...
      <li key={appointment._id} className="ViewAppointments-appointment-item">
        ...
        <img src="/images/BookAppointment/doctor.jpg" alt="Doctor" />
          ...
          <div className="appointment-details">
            <p><FaUserMd /> <strong>Doctor Name:</strong> {appointment.doctorId.fname} {appointment.doctorId.lname}</p>
            <p><FaCalendarAlt /> <strong>Date:</strong> {new Date(appointment.date).toLocaleDateString()}</p>
            <p><FaClock /> <strong>Time:</strong> {new Date(appointment.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
            <p><FaTimesCircle /> <strong>Status:</strong> Canceled</p>
          </div>
        </div>
      </li>
    ))}
...
<Modal show={showModal} onHide={handleModalClose} className="update-appointment-modal">
  ...
  <Modal.Footer className="modal-footer">
    <Button variant="secondary" onClick={handleModalClose} className="cancel-btn">
      Close
    </Button>
    <Button variant="primary" onClick={handleModalSave} className="save-btn">
      Save Changes
    </Button>
  </Modal.Footer>
</Modal>
